...
def isAscii(str):
    return bool(re.match(r"^[\x00-\xff]+$", str))

def isAsciiPunc(str):
    return bool(re.match(r"^[\x20-\x2f\x3a-\x40\x5b-\x60]+$", str))
...
def isAsciiPuncChar(char):
    charCode = ord(char)
    if (charCode >= 0x20 and charCode <= 0x2f) or (charCode >= 0x3a and charCode <= 0x40) or (charCode >= 0x5b and charCode <= 0x60):
        return True
    else:
        return False
